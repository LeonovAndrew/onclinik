...
<div class="doctor-reviews">
    ...
    <div class="doctor-reviews-btn-wrap">
        <a href="<?php echo getMessage('REVIEWS_LINK') . '?nameDoctor=' . $arResult['doctor']->name . '#feedback';?>" class="btn3"><?php echo getMessage('LEAVE_FEEDBACK');?></a>
        <?php
        if (!empty($arResult['doctor'])) {
            ?>
            <a href="<?php echo getMessage('REVIEWS_LINK') . '?search=' . $arResult['doctor']->name;?>" class="doctor-reviews-link"><?php echo getMessage('SEE_ALL_REVIEWS');?></a>
            <?php
        }
        ?>
    </div>
