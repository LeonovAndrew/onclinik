...
if($_REQUEST['work_start'] && check_bitrix_sessid())
{
	...
	$rs = $DB->Query("select * from $table where (ID>$lastID AND IBLOCK_ID=39) order by ID asc limit 500;");
	while ($ar = $rs->Fetch())
	{
		/*
		 * do something
	...
	$rsLeftBorder = $DB->Query("select ID from $table where (ID <= $lastID AND IBLOCK_ID=39) order by ID asc", false, "FILE: ".__FILE__."<br /> LINE: ".__LINE__);
	$leftBorderCnt = $rsLeftBorder->SelectedRowsCount();

	$rsAll = $DB->Query("select ID from $table where IBLOCK_ID=39;", false, "FILE: ".__FILE__."<br /> LINE: ".__LINE__);
