...
// Конпка вверх
	...
	/*var date = new Date(),   //текущая дата
	    ...
	    dayMonth = date.getDate(),   //текущий день
	    day = new Date(year, ++month, ++dayMonth).getDate();
	$('.clock').countdown([year, month, day].join('/')).on('update.countdown', function(event) {
		console.log('event');
		console.log(event);
		var test = new Date(event.timeStamp);
